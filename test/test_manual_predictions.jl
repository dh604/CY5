...
Qs = GW_CY5.load_omega_data("Omega_P2_111", [1, 3])
...
Qs2 = GW_CY5.load_omega_data("Omega_P2_111", 3)
println("Keys: ", sort(collect(keys(Qs2))))

println("\n=== Test 3: omega_exp_substitution ===")
Qs1 = GW_CY5.load_omega_data("Omega_P2_111", [1])
polys, t = GW_CY5.omega_exp_substitution(Qs1, 6)
println("Q1 deg=6: ", polys[1])

println("\n=== Test 4: omega_u_scaling ===")
R, (T0, T1, T2, T3, T4, u) = polynomial_ring(QQ, ["T0", "T1", "T2", "T3", "T4", "u"])
scaled = GW_CY5.omega_u_scaling(polys, [T0, T1, T2, T3, T4], u)
println("Keys: ", sort(collect(keys(scaled))))

println("\n=== Test 5: omega_series with collection ===")
result = GW_CY5.omega_series("Omega_P2_111", [1, 3], [T0, T1, T2, T3, T4], u, 6)
...
result2 = GW_CY5.omega_series("Omega_P2_111", 3, [T0, T1, T2, T3, T4], u, 6)
println("Keys: ", sort(collect(keys(result2))))

println("\n=== Test 7: consistency check ===")
Qs_all = GW_CY5.load_omega_data("Omega_P2_111", 3)
polys_all, _ = GW_CY5.omega_exp_substitution(Qs_all, 6)
scaled_all = GW_CY5.omega_u_scaling(polys_all, [T0, T1, T2, T3, T4], u)
