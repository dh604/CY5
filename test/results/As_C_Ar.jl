...
# run tests (up to genus 3)
# Note that "fails" means non-zero output and "holds" means output zero.

get_Omega_beta(G, [2*b], 3; check_predictions=true)
# fails for b, holds for 2b

get_Omega_beta(G, [2*B], 3; check_predictions=true)
# fails for B, holds for 2B

get_Omega_beta(G, [2*B + 2*b], 3; check_predictions=true)
# All predictions hold.

get_Omega_beta(G, [B + 2*b], 3; check_predictions=true)
# All predictions hold.

get_Omega_beta(G, [2*B + b], 3; check_predictions=true)
# All predictions hold.

get_Omega_beta(G, [3*B + 2*b], 3; check_predictions=true)
# All predictions hold.

get_Omega_beta(G, [2*b+2*c], 3; check_predictions=true)
# fails for b+c, holds for 2(b+c)

get_Omega_beta(G, [b+C], 3; check_predictions=true)
# All predictions hold.

get_Omega_beta(G, [b+B+C], 3; check_predictions=true)
# All predictions hold.

get_Omega_beta(G, [4*c], 3; check_predictions=true)
# fails for c, holds for 2c, 4c